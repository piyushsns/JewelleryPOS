...
const AddCustomerDrawer = ({ open, handleClose, setData, selectedRow, setSelectedRow }) => {
  ...
  const baseCustomerSchema = v.object({
  ...
  const fieldObjectArray = [
    { name: 'first_name', label: 'First Name', type: 'text', required: true, size: 12, classNames: '' },
    { name: 'last_name', label: 'Last Name', type: 'text', required: true, size: 12, classNames: '' },
    { name: 'email', label: 'Email', type: 'text', required: true, size: 12, classNames: '' },
    { name: 'gender', label: 'Gender', type: 'select', options: ['Male', 'Female', 'Other'], required: true, size: 12, classNames: '' },
    { name: 'phone', label: 'Contact Number', type: 'text', required: true, size: 12, classNames: '' },
  ];
  ...
  const onSubmit = async formData => {
    ...
    try {
      ...
      if (selectedRow) {
        res = await new HttpService().putData(formData, `admin/customers/${selectedRow.id}`, session?.user?.token);
      } else {
        res = await new HttpService().postData(formData, 'admin/customers', session?.user?.token);
      }
